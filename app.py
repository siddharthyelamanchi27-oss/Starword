...
def resolve_battle_answer(correct, label=""):
    ...
    if correct:
        ...
        st.session_state.feedback = f"⚡ {label} hits {st.session_state.monster_name}!"
        if qtype == "word":
        ...
        st.session_state.feedback = f"💨 {st.session_state.monster_name} dodges!{note}"
        st.session_state.current_move = None
...
def screen_battle():
    ...
    if st.session_state.catch_pending:
        if st.session_state.catch_result is None:
