...
.pop-number {
    font-size: 42px; font-weight: 900; text-align: center; margin: 8px 0;
...
.catch-banner {
    text-align:center; padding: 16px; border-radius: 14px; margin-bottom: 16px;
    background: #15151f; animation: popscale 0.5s ease-out;
}
.collection-card {
    text-align:center; background:#15151f; border-radius:12px; padding:12px;
...
def screen_menu():
    ...
    if st.button("🎙️ Voice Training Chamber (practice reading aloud)"):
        ...
        st.rerun()
    if st.button(f"📦 Creature Collection ({len(st.session_state.collection)} caught)"):
...
def screen_shop():
    ...
    if catch:
        ...
        <div class="catch-banner">
            <div style="font-size:70px;">{catch['emoji']}</div>
