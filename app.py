...
def generate_ai_question(qtype):
    """Ask Gemini for a fresh, randomized question of the given type for a 7-year-old.
    Returns a dict in the same shape as BOSS_QUESTIONS entries, or None on any failure."""
    key = get_gemini_key()
    if not key:
        return None
    type_instructions = {
    ...
    prompt = f"""Create {type_instructions.get(qtype, type_instructions['vocab'])} for a 7-year-old learning to read English.
Use simple, everyday, age-appropriate words only. Exactly 3 answer options, only one correct.
Respond with ONLY raw JSON, no markdown fences, matching this exact shape:
{{"type": "{qtype}", "prompt": "...", "passage": "..." (include only for comprehension type, omit otherwise), "options": ["...", "...", "..."], "answer": 0}}
"answer" is the 0-based index of the correct option."""
    try:
        ...
        if "options" in parsed and "answer" in parsed and "prompt" in parsed:
            parsed["type"] = qtype
...
def build_boss_round():
    ...
    ai_available = get_gemini_key() is not None
    questions = []
    for t in round_types:
        q = None
        if ai_available:
            q = generate_ai_question(t)
        if q is None:
